...
struct dynarray_handle *dynarray_handle_create() {
    return xxmalloc(sizeof(struct dynarray_handle));
}

void dynarray_handle_destroy(struct dynarray_handle *da_handle) {
    xxfree(da_handle);
}

struct fsm_state *fsm_state_init(struct dynarray_handle *da_handle, int sigma_size) {
    da_handle->current_fsm_head = xxmalloc(INITIAL_SIZE * sizeof(struct fsm_state));
    da_handle->current_fsm_size = INITIAL_SIZE;
    da_handle->current_fsm_linecount = 0;
    da_handle->ssize = sigma_size+1;
    da_handle->slookup = xxcalloc(da_handle->ssize*da_handle->ssize,sizeof(struct sigma_lookup));
    da_handle->mainloop = 1;
    da_handle->is_deterministic = 1;
    da_handle->is_epsilon_free = 1;
    da_handle->arccount = 0;
    da_handle->num_finals = 0;
    da_handle->num_initials = 0;
    da_handle->statecount = 0;
    da_handle->arity = 1;
    da_handle->current_trans = 1;
    return(da_handle->current_fsm_head);
}

void fsm_state_set_current_state(struct dynarray_handle *da_handle, int state_no, int final_state, int start_state) {
    da_handle->current_state_no = state_no;
    da_handle->current_final = final_state;
    da_handle->current_start = start_state;
    da_handle->current_trans = 0;
    if (da_handle->current_final == 1)
        da_handle->num_finals++;
    if (da_handle->current_start == 1)
	da_handle->num_initials++;
}

/* Add sentinel if needed */
void fsm_state_end_state(struct dynarray_handle *da_handle) {
    if (da_handle->current_trans == 0) {
        fsm_state_add_arc(da_handle, da_handle->current_state_no, -1, -1, -1, da_handle->current_final, da_handle->current_start);
    }
    da_handle->statecount++;
    da_handle->mainloop++;
}

void fsm_state_add_arc(struct dynarray_handle *da_handle, int state_no, int in, int out, int target, int final_state, int start_state) {
    ...
    if (in == EPSILON && out == EPSILON) {
        if (state_no == target) {
            ...
            da_handle->is_deterministic = 0;
            da_handle->is_epsilon_free = 0;
        }
    }
    ...
    if (in != -1 && out != -1) {
        if ((da_handle->slookup+(da_handle->ssize*in)+out)->mainloop == da_handle->mainloop) {
            if ((da_handle->slookup+(da_handle->ssize*in)+out)->target == target) {
	        return;
            } else {
	        da_handle->is_deterministic = 0;
            }
        ...
        da_handle->arccount++;
        (da_handle->slookup+(da_handle->ssize*in)+out)->mainloop = da_handle->mainloop;
        (da_handle->slookup+(da_handle->ssize*in)+out)->target = target;
    }
    
    da_handle->current_trans = 1;
    if (da_handle->current_fsm_linecount >= da_handle->current_fsm_size) {
        da_handle->current_fsm_size *= 2;
        da_handle->current_fsm_head = xxrealloc(da_handle->current_fsm_head, da_handle->current_fsm_size * sizeof(struct fsm_state));
        if (da_handle->current_fsm_head == NULL) {
            perror("Fatal error: out of memory\n");
            exit(1);
        }
    }
    cptr = da_handle->current_fsm_head + da_handle->current_fsm_linecount;
    ...
    da_handle->current_fsm_linecount++;
}

void fsm_state_close(struct dynarray_handle *da_handle, struct fsm *net) {
    fsm_state_add_arc(da_handle, -1,-1,-1,-1,-1,-1);
    da_handle->current_fsm_head = xxrealloc(da_handle->current_fsm_head, da_handle->current_fsm_linecount * sizeof(struct fsm_state));
    net->arity = da_handle->arity;
    net->arccount = da_handle->arccount;
    net->statecount = da_handle->statecount;
    net->linecount = da_handle->current_fsm_linecount;
    net->finalcount = da_handle->num_finals;
    net->pathcount = PATHCOUNT_UNKNOWN;
    if (da_handle->num_initials > 1)
	da_handle->is_deterministic = 0;
    net->is_deterministic = da_handle->is_deterministic;
    net->is_pruned = UNK;
    net->is_minimized = UNK;
    net->is_epsilon_free = da_handle->is_epsilon_free;
    net->is_loop_free = UNK;
    net->is_completed = UNK;
    net->arcs_sorted_in = 0;
    net->arcs_sorted_out = 0;

    net->states = da_handle->current_fsm_head;
    xxfree(da_handle->slookup);
}
...
struct fsm *fsm_construct_done(struct dynarray_handle *da_handle, struct fsm_construct_handle *handle) {
    int i, emptyfsm;
    struct fsm *net;
    struct fsm_state_list *sl;
    ...
    fsm_state_init(da_handle, (handle->maxsigma)+1);

    for (i=0, emptyfsm = 1; i <= handle->maxstate; i++) {
        fsm_state_set_current_state(da_handle, i, (sl+i)->is_final, (sl+i)->is_initial);
	if ((sl+i)->is_initial && (sl+i)->is_final)
	    ...
	    if ((sl+i)->is_initial)
		emptyfsm = 0;
            fsm_state_add_arc(da_handle, i, trans->in, trans->out, trans->target, (sl+i)->is_final, (sl+i)->is_initial);
        }
        fsm_state_end_state(da_handle);
    }
    net = fsm_create("");
    sprintf(net->name, "%X",rand());
    fsm_sigma_destroy(&net->sigma);
    fsm_state_close(da_handle, net);

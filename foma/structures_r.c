...
struct build_handle *build_handle_create(int stack_size, int ptr_stack_size) {
    struct build_handle *b_handle = xxmalloc(sizeof(struct build_handle));
    b_handle->stack = ptr_stack_handle_create(stack_size, ptr_stack_size);
    b_handle->da_handle = dynarray_handle_create();
...
void build_handle_destroy(struct build_handle *b_handle) {
    ...
    ptr_stack_handle_destroy(b_handle->stack);
    dynarray_handle_destroy(b_handle->da_handle);
...
struct fsm *fsm_sigma_net(struct build_handle *b_handle, struct fsm *net) {
    ...
    fsm_state_set_current_state(b_handle->da_handle, 0, 0, 1);
    pathcount = 0;
    for (unsigned int i = 0; i < net->sigma.size; ++i) {
        if (syms[i].number >= 3 || syms[i].number == IDENTITY) {
            pathcount++;
            fsm_state_add_arc(b_handle->da_handle, 0, syms[i].number, syms[i].number, 1, 0, 1);
        }
    }
    fsm_state_end_state(b_handle->da_handle);
    fsm_state_set_current_state(b_handle->da_handle, 1, 1, 0);
    fsm_state_end_state(b_handle->da_handle);
    xxfree(net->states);
...
struct fsm *fsm_sigma_pairs_net(struct build_handle *b_handle, struct fsm *net) {
    ...
    for (fsm = net->states, i=0; (fsm+i)->state_no != -1; i++) {
        ...
        if (*(pairs+smax*in+out) == 0) {
            fsm_state_add_arc(b_handle->da_handle, 0,in,out, 1, 0, 1);
            *(pairs+smax*in+out) = 1;
            pathcount++;
        }
    }
...
int fsm_isuniversal(struct build_handle *b_handle, struct fsm *net) {
    struct fsm_state *fsm;
    net = fsm_minimize(b_handle, net);
    fsm_compact(b_handle, net);
    fsm = net->states;
...
int fsm_isempty(struct build_handle *b_handle, struct fsm *net) {
    struct fsm_state *fsm;
    net = fsm_minimize(b_handle, net);
    fsm = net->states;
...
int fsm_isfunctional(struct build_handle *b_handle, struct fsm *net) {
    return(fsm_isidentity(b_handle, fsm_minimize(b_handle,fsm_compose(b_handle,fsm_invert(fsm_copy(net)),fsm_copy(net)))));
}

int fsm_isunambiguous(struct build_handle *b_handle, struct fsm *net) {
    struct fsm *loweruniqnet, *testnet;
    int ret;
    loweruniqnet = fsm_lowerdet(b_handle, fsm_copy(net));
    testnet = fsm_minimize(b_handle,fsm_compose(b_handle,fsm_invert(fsm_copy(loweruniqnet)),fsm_copy(loweruniqnet)));
    ret = fsm_isidentity(b_handle, testnet);
    fsm_destroy(loweruniqnet);
    fsm_destroy(testnet);
    return(ret);
}

struct fsm *fsm_extract_ambiguous_domain(struct build_handle *b_handle, struct fsm *net) {
    // define AmbiguousDom(T) [_loweruniq(T) .o. _notid(_loweruniq(T).i .o. _loweruniq(T))].u;
    struct fsm *loweruniqnet, *result;
    loweruniqnet = fsm_lowerdet(b_handle, net);
    result = fsm_topsort(b_handle, fsm_minimize(b_handle,fsm_upper(b_handle, fsm_compose(b_handle,fsm_copy(loweruniqnet),fsm_extract_nonidentity(b_handle, fsm_compose(b_handle,fsm_invert(fsm_copy(loweruniqnet)), fsm_copy(loweruniqnet)))))));
    fsm_destroy(loweruniqnet);
...
struct fsm *fsm_extract_ambiguous(struct build_handle *b_handle, struct fsm *net) {
    struct fsm *result;
    result = fsm_topsort(b_handle, fsm_minimize(b_handle,fsm_compose(b_handle,fsm_extract_ambiguous_domain(b_handle,fsm_copy(net)), net)));
    return(result);
}

struct fsm *fsm_extract_unambiguous(struct build_handle *b_handle, struct fsm *net) {
    struct fsm *result;
    result = fsm_topsort(b_handle, fsm_minimize(b_handle,fsm_compose(b_handle,fsm_complement(b_handle, fsm_extract_ambiguous_domain(b_handle,fsm_copy(net))), net)));
    return(result);
}

int fsm_isidentity(struct build_handle *b_handle, struct fsm *net) {
    ...
    ptr_stack_clear(b_handle->stack);
    ptr_stack_push(b_handle->stack, state_array->transitions);

    while(!ptr_stack_isempty(b_handle->stack)) {

        curr_ptr = ptr_stack_pop(b_handle->stack);
    ...
    nopop:
        ...
        if (curr_ptr->state_no == (curr_ptr+1)->state_no) {
            ptr_stack_push(b_handle->stack, curr_ptr+1);
        }
...
struct fsm *fsm_lowerdet(struct build_handle *b_handle, struct fsm *net) {
...
struct fsm *fsm_lowerdeteps(struct build_handle *b_handle, struct fsm *net) {
    unsigned int newsym; /* Running number for new syms */
    struct fsm_state *fsm;
    char repstr[13];
    int i,j,maxsigma,maxarc;
...
struct fsm *fsm_extract_nonidentity(struct build_handle *b_handle, struct fsm *net) {
    ...
    ptr_stack_clear(b_handle->stack);
    sigma_sort(net);
    net2 = fsm_upper(b_handle, fsm_compose(b_handle,net,fsm_contains(b_handle,fsm_symbol("@KILL@"))));
...
struct fsm *fsm_quantifier(struct build_handle *b_handle, char *string) {

    /* \x* x \x* x \x* */
    return(fsm_concat(b_handle,fsm_kleene_star(b_handle,fsm_term_negation(b_handle,fsm_symbol(string))),fsm_concat(b_handle,fsm_symbol(string),fsm_concat(b_handle,fsm_kleene_star(b_handle,fsm_term_negation(b_handle,fsm_symbol(string))),fsm_concat(b_handle,fsm_symbol(string),fsm_kleene_star(b_handle,fsm_term_negation(b_handle,fsm_symbol(string))))))));

}

struct fsm *fsm_logical_precedence(struct build_handle *b_handle, char *string1, char *string2) {
    /* x < y = \y* x \y* [x | y Q* x] ?* */
    /*          1  2  3        4           5 */

    return(fsm_concat(b_handle,fsm_kleene_star(b_handle,fsm_term_negation(b_handle,fsm_symbol(string2))),fsm_concat(b_handle,fsm_symbol(string1),fsm_concat(b_handle,fsm_kleene_star(b_handle,fsm_term_negation(b_handle,fsm_symbol(string2))),fsm_concat(b_handle,fsm_union(fsm_symbol(string1),fsm_concat(b_handle,fsm_symbol(string2),fsm_concat(b_handle,union_quantifiers(),fsm_symbol(string1)))),fsm_universal())))));

/*    1,3   fsm_kleene_star(b_handle,fsm_term_negation(b_handle,fsm_symbol(string2))) */
/*        2 = fsm_symbol(string1) */
/*        5 = fsm_universal() */
/* 4 =    fsm_union(fsm_symbol(string1),fsm_concat(b_handle,fsm_symbol(string2),fsm_concat(b_handle,union_quantifiers(),fsm_symbol(string1)))) */

}

/** Logical equivalence, i.e. where two variables span the same substring */
/** x = y = ?* [x y|y x]/Q ?* [x y|y x]/Q ?* */
struct fsm *fsm_logical_eq(struct build_handle *b_handle,char *string1, char *string2) {
  return(fsm_concat(b_handle,fsm_universal(),fsm_concat(b_handle,fsm_ignore(b_handle,fsm_union(fsm_concat(b_handle,fsm_symbol(string1),fsm_symbol(string2)),fsm_concat(b_handle,fsm_symbol(string2),fsm_symbol(string1))),union_quantifiers(),OP_IGNORE_ALL),fsm_concat(b_handle,fsm_universal(),fsm_concat(b_handle,fsm_ignore(b_handle,fsm_union(fsm_concat(b_handle,fsm_symbol(string1),fsm_symbol(string2)),fsm_concat(b_handle,fsm_symbol(string2),fsm_symbol(string1))),union_quantifiers(),OP_IGNORE_ALL),fsm_universal())))));
}

...
struct dynarray_handle {
    size_t current_fsm_size;
    unsigned int current_fsm_linecount, current_state_no, current_final, current_start, current_trans, num_finals, num_initials, arity, statecount;
    _Bool is_deterministic, is_epsilon_free;
    struct fsm_state *current_fsm_head;

    unsigned int mainloop, ssize, arccount;

    struct sigma_lookup *slookup;
};

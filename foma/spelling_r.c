...
void print_match(struct build_handle *b_handle, struct apply_med_handle *medh, struct astarnode *node, struct sigma *sigma, char *word) {
    ...
    for (n = node; n != NULL ; n = medh->agenda+(n->parent)) {
        ...
        if (n->parent == -1)
            break;
        int_stack_push(b_handle->stack, n->in);
    }
    printptr = 0;
    ...
    while (!(int_stack_isempty(b_handle->stack))) {
        sym = int_stack_pop(b_handle->stack);
        if (sym > 2) {
    ...
    for (n = node; n != NULL ; n = medh->agenda+(n->parent)) {
        ...
        if (n->parent == -1)
            ...
            int_stack_push(b_handle->stack, n->out);
    }
    printptr = 0;
    ...
    for (i = 0; !(int_stack_isempty(b_handle->stack)); ) {
        sym = int_stack_pop(b_handle->stack);	
        if (sym > 2) {
...
char *apply_med(struct build_handle *b_handle, struct apply_med_handle *medh, char *word) {
...
void fsm_create_letter_lookup(struct build_handle *b_handle, struct apply_med_handle *medh, struct fsm *net) {
    ...
    l1:
        ...
        index++;
        int_stack_push(b_handle->stack, v);
    ...
    l2:
        ...
        if ((sccinfo+vp)->index == 0) {
            ...
            ptr_stack_push(b_handle->stack, curr_ptr);
    ...
    l4:
        if ((sccinfo+v)->lowlink == (sccinfo+v)->index) {
            //printf("\nSCC: [%i] ",v);
            while((copystate = int_stack_pop(b_handle->stack)) != v) {
    ...
    for (v=0; v < num_states; v++) {
        ptr_stack_push(b_handle->stack, (medh->state_array+v)->transitions);
        int_stack_push(b_handle->stack, 0);
        while (!ptr_stack_isempty(b_handle->stack)) {
            curr_ptr = ptr_stack_pop(b_handle->stack);
            depth = int_stack_pop(b_handle->stack);
        looper:
            ...
            if (curr_ptr->target != -1) {
                if (curr_ptr->state_no == (curr_ptr+1)->state_no) {
                    ...
                    int_stack_push(b_handle->stack, depth);
                }
                depth++;
                curr_ptr = (medh->state_array+(curr_ptr->target))->transitions;
                goto looper;

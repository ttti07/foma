...
struct fsm *fsm_topsort (struct build_handle *b_handle, struct fsm *net) {
    ...
    *(pathcount+0) = 1;
    ...
    overflow = 0;
    for (i=0 ; !int_stack_isempty(b_handle->stack); i++) {
        /* Treat a state */
        curr_state = int_stack_pop(b_handle->stack);
        ...
        while (curr_fsm->state_no == curr_state) {
            if (curr_fsm->target != -1 ) {
                ...
                if ( *(invcount+(curr_fsm->target)) == 0) {
                    int_stack_push(b_handle->stack, curr_fsm->target);
                }
            }
            curr_fsm++;
        }       

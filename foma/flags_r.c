...
struct fsm *flag_eliminate(struct build_handle *b_handle, struct fsm *net, char *name) {
    ...
    for (f = flags; f != NULL; f = f->next) {
        ...
        if ((name == NULL || strcmp(f->name,name) == 0) &&
            ...
            for (ff = flags, flag = 0; ff != NULL; ff = ff->next) {
                ...
                if (fstatus == FAIL) {
                    fail_flags = fsm_minimize(b_handle, fsm_union(fail_flags, flag_create_symbol(ff->type, ff->name, ff->value)));
                    flag = 1;
                }
                if (fstatus == SUCCEED) {
                    succeed_flags = fsm_minimize(b_handle, fsm_union(succeed_flags, flag_create_symbol(ff->type, ff->name, ff->value)));
                    flag = 1;
                }
            }
        }
        ...
        if (flag) {
            if (f->type == FLAG_REQUIRE) {
                newfilter = fsm_complement(b_handle, fsm_concat(b_handle, fsm_optionality(b_handle, fsm_concat(b_handle, fsm_universal(), fail_flags)), fsm_concat(b_handle, fsm_complement(b_handle, fsm_contains(b_handle, succeed_flags)), fsm_concat(b_handle, self, fsm_universal()))));
                
            } else {
                newfilter = fsm_complement(b_handle, fsm_contains(b_handle, fsm_concat(b_handle, fail_flags,fsm_concat(b_handle, fsm_complement(b_handle, fsm_contains(b_handle, succeed_flags)),self))));
            }

            filter = (filter == NULL) ? newfilter : fsm_intersect(b_handle, filter, newfilter);
        }
        flag = 0;
    }
    if (filter != NULL) {
        ...
        g_flag_is_epsilon = 0;
        newnet = fsm_compose(b_handle, fsm_copy(filter),fsm_compose(b_handle, net,fsm_copy(filter)));
        g_flag_is_epsilon = old_g_flag_is_epsilon;
    } else {
        newnet = net;
    }
    flag_purge(newnet, name);
    newnet = fsm_minimize(b_handle, newnet);
    sigma_cleanup(newnet,0);
    sigma_sort(newnet);
    xxfree(flags);
    return(fsm_topsort(b_handle, newnet));
}
...
struct fsm *flag_twosided(struct build_handle *b_handle, struct fsm *net) {
  ...
  if (newarcs == 0) {
    if (change == 1) {
      ...
      net->is_pruned = UNK;
      return fsm_topsort(b_handle, fsm_minimize(b_handle, net));
    }
    return net;
  }

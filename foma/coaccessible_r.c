...
struct fsm *fsm_coaccessible(struct build_handle *b_handle, struct fsm *net) {
  ...
  for (i=0; (fsm+i)->state_no != -1; i++) {
    if ((fsm+i)->final_state && (!*(coacc+((fsm+i)->state_no)))) {
      int_stack_push(b_handle->stack, (fsm+i)->state_no);
  ...
  while(!int_stack_isempty(b_handle->stack)) {
    current_state = int_stack_pop(b_handle->stack);
	...
	*(coacc+(current_ptr->state)) = 1;
	int_stack_push(b_handle->stack, current_ptr->state);
	markcount++;
      }
